...
use crate::scheduler::{PerCoreSchedulerExt, on_load_baln_timer, on_load_calc_timer, timer_interrupts};
#[repr(C, packed)]
struct State {
...
extern "x86-interrupt" fn timer_handler(stack_frame: interrupts::ExceptionStackFrame) {
    ...
    let fired = timer_interrupts::clear_active_and_set_next();

    if fired == timer_interrupts::Source::LoadCalculator {
        on_load_calc_timer(core_scheduler());
    }
    if fired == timer_interrupts::Source::LoadBalancer {
        on_load_baln_timer(core_scheduler());
    }
	debug!("Handle timer interrupt {}", apic::TIMER_INTERRUPT_NUMBER);
	// timer_interrupts::clear_active_and_set_next();

	core_scheduler().handle_waiting_tasks();
	apic::eoi();
    match fired {
        timer_interrupts::Source::LoadBalancer | timer_interrupts::Source::LoadCalculator => {
            // Nothing to be done
        }
        _ => {
