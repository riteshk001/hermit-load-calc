...
use timer_interrupts::{TimerList, Source};
...
use crate::scheduler::timer_interrupts::create_timer_abs;
use crate::io;

pub mod task;
pub mod timer_interrupts;
pub mod load_calculate;

/// Interval between load-average samples, in microseconds.
const LOAD_BALN_INTERVAL_US: u64 = 1_000_000;

const LOAD_CALC_INTERVAL_US: u64 = 100_000;
/// Imbalance threshold around the mean avg_5.
#[cfg(feature = "smp")]
const LOAD_BALANCE_MARGIN: f64 = 0.5;
/// Total tasks that may be migrated in one balancer tick.
#[cfg(feature = "smp")]
const MAX_MIGRATIONS_PER_TICK: usize = 64;
/// Max tasks moved for a single donor→receiver pairing.
#[cfg(feature = "smp")]
const MAX_TASKS_PER_PAIR: usize = 16;
/// Leave at least this many ready tasks on a donor.
#[cfg(feature = "smp")]
const MIN_READY_TO_DONATE: usize = 2;
...
/// Cross-core ownership transfer of an existing task.
///
/// Tasks normally live as `Rc` on a single core. Migration removes the task
/// from the source ready queue first, then hands exclusive ownership to the
/// destination via `SchedulerInput`. That exclusive transfer is what makes
/// this `Send` wrapper sound.
#[cfg(feature = "smp")]
struct MigratedTask(Rc<RefCell<Task>>);

#[cfg(feature = "smp")]
unsafe impl Send for MigratedTask {}
...
pub(crate) struct SchedulerInput {
	...
	/// Ready tasks migrated here by the load balancer.
	migrated_tasks: VecDeque<MigratedTask>,
	/// Destinations this core should donate to, with how many tasks to send.
	donate_to: VecDeque<(CoreId, usize)>,
}
...
impl SchedulerInput {
	pub fn new() -> Self {
		Self {
			...
			migrated_tasks: VecDeque::new(),
			donate_to: VecDeque::new(),
		}
	}
}
...
impl PerCoreScheduler {
	...
	pub fn check_input(&mut self) {
		let donate_targets = {
			...
			while let Some(MigratedTask(task)) = input_locked.migrated_tasks.pop_front() {
				// info!(
				// 	"Core {} accepted migrated task {}",
				// 	self.core_id,
				// 	task.borrow().id
				// );
				self.ready_queue.push(task);
			...
			while let Some(request) = input_locked.donate_to.pop_front() {
				targets.push(request);
			}
			targets
		};

		for (dst, count) in donate_targets {
			try_migrate_local_tasks(self, dst, count);
		}
	}

	/// Count runnable and blocked tasks assigned to this core.
	pub(crate) fn count_active_tasks(&self) -> u32 {
		let mut count = self.ready_queue.len() as u32;

		#[cfg(feature = "smp")]
		{
			let input = get_scheduler_input(self.core_id).lock();
			count += input.new_tasks.len() as u32;
			count += input.wakeup_tasks.len() as u32;
			count += input.migrated_tasks.len() as u32;
		}
		// if self.current_task.borrow().status == TaskStatus::Running {
		//     count += 1;
		// }

		// count += self.blocked_tasks.len() as u32;
		count
	}
	...
	pub fn run() -> ! {
		...
		create_timer_abs(
			Source::LoadCalculator,
			processor::get_timer_ticks() + LOAD_CALC_INTERVAL_US,
		);
        /// Create balancer timer only on the 0th core
        if core_id() == 0 {
            create_timer_abs(
                Source::LoadBalancer,
                processor::get_timer_ticks() + LOAD_BALN_INTERVAL_US,
            );
        }
...
pub(crate) fn on_load_calc_timer(scheduler: &PerCoreScheduler) {
    load_calculate::update_for_core(scheduler);
    create_timer_abs(Source::LoadCalculator, processor::get_timer_ticks() + LOAD_CALC_INTERVAL_US);
}

pub(crate) fn on_load_baln_timer(scheduler: &mut PerCoreScheduler) {
    #[cfg(feature = "smp")]
    if core_id() == 0 {
        balance_load_across_cores(scheduler);
    }
...
#[cfg(feature = "smp")]
fn balance_load_across_cores(scheduler: &mut PerCoreScheduler) {
	use fixed::types::U53F11;

	let num_cores = get_processor_count();
	if num_cores <= 1 {
		trace!("Cores <= 1, no need for load balancing.");
		return;
	}

	let loads = load_calculate::collect_cpu_loads(num_cores);
	if loads.len() < 2 {
		return;
	}

	// Mean avg_5 across sampled cores.
	let mut sum = U53F11::from_num(0);
	for (_, load) in &loads {
		sum += load.avg_5;
	}
	let mean = sum / U53F11::from_num(loads.len() as u32);
	let margin = U53F11::from_num(LOAD_BALANCE_MARGIN);

	// Donors: clearly above mean. Receivers: clearly below mean.
	let mut donors: Vec<(CoreId, U53F11)> = Vec::new();
	let mut receivers: Vec<(CoreId, U53F11)> = Vec::new();

	for (core, load) in &loads {
		if load.avg_5 > mean + margin {
			donors.push((*core, load.avg_5));
		} else if load.avg_5 + margin < mean {
			receivers.push((*core, load.avg_5));
		}
	}

	if donors.is_empty() || receivers.is_empty() {
		trace!(
			"No donors/receivers around mean={:.2}",
			mean.to_num::<f32>()
		);
		return;
	}

	// Heaviest donors first, lightest receivers first.
	donors.sort_by(|a, b| b.1.cmp(&a.1));
	receivers.sort_by(|a, b| a.1.cmp(&b.1));

	// info!(
	// 	"Balance: mean={:.2} donors={} receivers={} budget={}",
	// 	mean.to_num::<f32>(),
	// 	donors.len(),
	// 	receivers.len(),
	...
	let mut remaining = MAX_MIGRATIONS_PER_TICK;
	let mut donor_idx = 0usize;
	let mut recv_idx = 0usize;
	let mut stalled = 0usize;

	while remaining > 0 && stalled < donors.len() {
		let src = donors[donor_idx].0;
		let dst = receivers[recv_idx].0;

		// Move more tasks from heavier donors: scale by how far above mean.
		let excess = donors[donor_idx].1 - mean;
		let estimated = excess.to_num::<u32>() as usize;
		let batch = estimated.clamp(1, MAX_TASKS_PER_PAIR).min(remaining);

		// info!(
		// 	"Pairing donor core {src} (avg_5={:.2}) → receiver core {dst} (avg_5={:.2}), batch={batch}",
		// 	donors[donor_idx].1.to_num::<f32>(),
		// 	receivers[recv_idx].1.to_num::<f32>(),
		// );

		let moved = request_or_do_migrate(scheduler, src, dst, batch);
		if moved == 0 {
			stalled += 1;
		} else {
			stalled = 0;
		...
		donor_idx = (donor_idx + 1) % donors.len();
		// Round-robin receivers so one idle core is not flooded.
		recv_idx = (recv_idx + 1) % receivers.len();
	}
}

/// Ask `src` to donate up to `count` ready tasks to `dst`.
/// Returns how many tasks were migrated locally (0 for remote requests).
#[cfg(feature = "smp")]
fn request_or_do_migrate(
	...
	if scheduler.core_id == src {
		try_migrate_local_tasks(scheduler, dst, count)
	} else {
		get_scheduler_input(src)
			.lock()
			.donate_to
			.push_back((dst, count));
		kernel::wakeup_core(src);
		// info!("Requested donation of {count} task(s) from core {src} to {dst}");
		// Treat the request as consuming budget; actual move happens on src.
		count
	}
}

/// Pop up to `count` ready tasks from this core and send them to `dst_core`.
/// Returns the number of tasks actually migrated.
#[cfg(feature = "smp")]
fn try_migrate_local_tasks(
	scheduler: &mut PerCoreScheduler,
	dst_core: CoreId,
	count: usize,
) -> usize {
	if dst_core == scheduler.core_id || count == 0 {
		return 0;
	}

	let mut moved = 0usize;
	let mut wake_dst = false;

	while moved < count {
		// Keep surplus on the donor so we don't empty it.
		if scheduler.ready_queue.len() < MIN_READY_TO_DONATE {
			break;
		}

		let Some(task) = scheduler.ready_queue.pop() else {
			break;
		};

		let (tid, prio, src_core) = {
			let mut borrowed = task.borrow_mut();
			assert_ne!(
				borrowed.status,
				TaskStatus::Idle,
				"idle task must not be migrated"
			);
			let src = borrowed.core_id;
			borrowed.core_id = dst_core;
			(borrowed.id, borrowed.prio, src)
		};

		{
			let mut tasks = TASKS.lock();
			tasks.insert(tid, TaskHandle::new(tid, prio, dst_core));
		}

		get_scheduler_input(dst_core)
			.lock()
			.migrated_tasks
			.push_back(MigratedTask(task));

		// info!("Migrated task {tid}: core {src_core} → {dst_core}");
		moved += 1;
		wake_dst = true;
	}

	if wake_dst {
		kernel::wakeup_core(dst_core);
	} else if count > 0 {
		// info!(
		// 	"Skip migrate on core {}: ready_queue.len()={}",
		// 	scheduler.core_id,
		// 	scheduler.ready_queue.len()
		// );
	}
