...
impl TimerList {
	pub fn new() -> Self {
		Self([
            ...
            Slot {
                source: Source::LoadCalculator,
                wakeup_time: u64::MAX,
            },
            Slot {
                source: Source::LoadBalancer,
                wakeup_time: u64::MAX,
            },
		])
	}
...
pub fn clear_active_and_set_next() -> Source {
	let timers = &mut core_scheduler().timers;
    ...
    let fired_source = lowest_timer.source;
	match lowest_timer.source {
        Source::LoadCalculator => {
            // Callback function in handle_timer()
        },
        Source::LoadBalancer => {
            //TODO: Add callback function to call load comparison across cores.
        },
		#[cfg(feature = "net")]
	...
	debug!("Cleared active timer {lowest_timer:?}");
